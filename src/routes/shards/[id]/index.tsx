import { $, component$, type NoSerialize, noSerialize, useSignal, useStore, useTask$, useVisibleTask$, type QwikMouseEvent } from '@builder.io/qwik';
...
const getGroupsAndSectors = () => {
    const playerWrappers = paper.project.activeLayer.children.filter(child => child.name?.includes('p-wrapper'));
    const groups = playerWrappers.flatMap(wrapper => wrapper.children.filter(wrapperChild => wrapperChild.name.includes('group')));
    const sectors = playerWrappers.flatMap(wrapper => wrapper.children.filter(wrapperChild => wrapperChild.name.includes('sector')));
...
const sectorIntersectionAreas: { 
    [authorId: number]: { 
        [id: number]: paper.PathItem | undefined 
    } 
}= {};

const checkSectorIntersections = (currentAuthorId: number) => {
    const sectors = getGroupsAndSectors().sectors;
    const sector = sectors.find(s => {
        return s.id === currentAuthorId;
    }) as paper.Path.Arc | undefined;
    ...
    sectorArray.forEach(savedSector => {
        ...
        if (sector.intersects(savedSector)) {
            ...
            const playerWrapper = sector.parent;

            if (playerWrapper.id === playerWrapper.parent.lastChild.id) {
                //console.log('already at front');
            } else {
                playerWrapper.bringToFront();
            }
...
interface DrawPlayerProps {
    ...
    index: number,
    theme: 'dark' | 'light', 
    initialGroupData?: ExtractedPlayerData
}
type CustomPositionItem = (
    paper.Path.Circle | 
    paper.Path.Arc |
    paper.Path.Ellipse | 
    paper.Path.Line
...
function cloneProxy<T>(proxyObject: T): T {
    ...
    if (Array.isArray(proxyObject)) {
      // If it's an array, clone its elements
        const clonedArray: any[] = [];
        for (let i = 0; i < proxyObject.length; i++) {
            clonedArray[i] = cloneProxy(proxyObject[i]);
        }
        return clonedArray as unknown as T;
    }

    // Create a new object with the same prototype
    const clonedObject: any = Object.create(Object.getPrototypeOf(proxyObject));

    // Iterate over the properties of the proxyObject and copy them to the clone
    for (const key in proxyObject) {
        if (Object.prototype.hasOwnProperty.call(proxyObject, key)) {
            clonedObject[key] = cloneProxy(proxyObject[key]);
        }
    }
...
const drawPlayer = $((props: DrawPlayerProps) => {
    ...
    const playerCircle = new paper.Path.Circle(point, playerCircleRadius) as CustomPositionItem;
    playerCircle.applyMatrix = false; // Enables animations
    playerCircle.name = `p-circle-${index}`;

    if (offense) { // if the player is offense
        playerCircle.fillColor = new paper.Color(255 / 255, 255 / 255, 255 / 255, 1);
    } else {
        playerCircle.fillColor = new paper.Color('#1d1d1d');
    }

    if (theme == 'light' && offense) {
        playerCircle.strokeColor = new paper.Color('#1d1d1d');
    }
    
    // Create Player Group
    const playerGroup = new paper.Group();
    playerGroup.applyMatrix = false; 
    playerGroup.name = `p-${offense ? 'offense' : 'defense'}-group-${index}`;
    ...
    const throughCircle = new paper.Path.Circle(arcThroughPoint.add(new paper.Point(0, 0)), 10) as CustomPositionItem;
    ...
    const arcEndPoint = new paper.Point(
        center.x + (radius + sectorRadius) * Math.cos(endAngle * (Math.PI / 180)),
        center.y + (radius + sectorRadius) * -Math.sin(endAngle * (Math.PI / 180))
    );

    
    const sector = Object.assign(new paper.Path.Arc(arcStartPoint, arcThroughPoint, arcEndPoint), {
        applyMatrix: false,
        fillColor: theme == 'dark' ? new paper.Color(0, 0, 0, 1) : new paper.Color(1, 1, 1, 1),
        strokeWidth: 1,
        strokeColor: new paper.Color('#4d4d4d')
    ...
    sector.lineTo(arcStartPoint);
    sector.closePath();
    sector.sendToBack();
    sector.name = `p-sector-${index}`;
...
/*     const sectorText = Object.assign(new paper.PointText(sector.bounds.center), {
    ...
    playerNumber.name = `p-number`

    const playerCircleGroup = new paper.Group([playerCircle, playerNumber])
    playerCircleGroup.name = `p-circle-group`;

    playerGroup.addChild(playerCircleGroup)
    ...
    playerGroup.addChildren([hinterArc, arrowheadStart, arrowheadEnd])
    playerGroup.addChild(throughCircle)

    const masterGroup = new paper.Group([
        sector,
        playerGroup,
    ])

    masterGroup.name = `p-wrapper-${offense ? 'offense' : 'defense'}-${index}`


    masterGroup.onMouseEnter = function () {
        highlightPlayer(index);
    }
    masterGroup.onMouseLeave = function () {
        if (isMouseDown) return;
        unHighlightPlayer(index);
    }

    playerCircleGroup.onMouseEnter = function () {
        if (areHandlersDisabled()) return;

        if (playerGroup.id === playerGroup.parent.lastChild.id) {
            console.log('already at front');
        } else {
    ...
    playerCircle._customPosition = {
        x: playerCircle.position.x,
        y: playerCircle.position.y
    }

    throughCircle._customPosition = {
        x: throughCircle.position.x,
        y: throughCircle.position.y
    }

    if (initialGroupData) {
        playerCircle._customPosition = initialGroupData.playerCircle.position;
        throughCircle._customPosition = initialGroupData.throughCircle.position;
        //console.log('INITIAL EXISTS')
    }

    console.log({throughCircle, offense})
    //const testPoint2 = new paper.Point(playerCircle._customPosition.x, playerCircle._customPosition.y);
    //const test2 = new paper.Path.Circle(testPoint2, 10);
    //test2.fillColor = new paper.Color(offense ? 'red' : 'blue');
    
    sectorArray.push(sector);

    const testPoint = new paper.Point(
        playerCircle._customPosition.x,
        playerCircle._customPosition.y
    )
    const test = new paper.Path.Circle(testPoint, 10);
    test.fillColor = new paper.Color('transparent');
    ...
    const playerCircleMouseMoveHandler = (e: any) => {
        ...
        playerGroup.position.y += deltaY;

        playerCircle._customPosition.x += deltaX;
        playerCircle._customPosition.y += deltaY;


        sector.position.x += deltaX;
        sector.position.y += deltaY;

        throughCircle._customPosition.x += deltaX;
        throughCircle._customPosition.y += deltaY;

        writeVar('circle-x', playerCircle._customPosition.x);
        writeVar('circle-y', playerCircle._customPosition.y);

        const deltaPoint = new paper.Point(deltaX, deltaY)
        moveHighlight(index, deltaPoint);
    ...
    const throughCircleDocumentMouseMoveHandler = (e: any) => {
        ...
        const pivotOrigin = playerCircle._customPosition;
        
        const x = throughCircle._customPosition.x - pivotOrigin.x;
        const y = throughCircle._customPosition.y - pivotOrigin.y;

        const targetAngleInRadians = Math.atan2(y, x);
        const targetAngleInDegrees = targetAngleInRadians * (180 / (Math.PI));

        const currentAngle = playerGroup.rotation;
        const actualTargetAngleInDegrees = targetAngleInDegrees + 90;

        const moveByAngle = (actualTargetAngleInDegrees - currentAngle);

        playerGroup.rotate(moveByAngle, pivotOrigin);
        ...
        document.body.style.cursor = 'grabbing';
        console.log(throughCircle._customPosition)
    }

    if (!offense) {
        playerGroup.rotate(180, playerCircle._customPosition);
        sector.rotate(180, playerCircle._customPosition);
        playerNumber.rotate(-180);// Weird

        throughCircle._customPosition.y += 300; // RADIUS IS 300 
        // HUH??
    }
    ...
    positionalsButton.addEventListener('click', () => {
        console.log(`group: ${playerGroup.id}`, {
            group: {
            ...
            savedPlayerCirclePosition: playerCircle._customPosition,
            savedThroughCirclePosition: throughCircle._customPosition
        })
    })

    // MANIPULATION, HANDLERS, DATA

    console.groupCollapsed('handlers')
    console.log({initialGroupData, offense})

    if (initialGroupData && initialGroupData.sector) {
        const playerSectorPoint = addDistanceToFieldCenter(new paper.Point(
            initialGroupData.sector.position.x,
            initialGroupData.sector.position.y 
        )) 
        sector.position.x = playerSectorPoint.x;
        sector.position.y = playerSectorPoint.y;
        sector.rotation = initialGroupData.group.rotation
    }

    if (initialGroupData && initialGroupData.group) {
        const innerGroupPoint = addDistanceToFieldCenter(new paper.Point(
            initialGroupData.group.position.x,
            initialGroupData.group.position.y 
        )) 
        playerGroup.position.x = innerGroupPoint.x;
        playerGroup.position.y = innerGroupPoint.y;

        console.log({playerGroup}, initialGroupData.group.position.y)

        playerGroup.rotation = initialGroupData.group.rotation
        playerNumber.rotation = -initialGroupData.group.rotation;


        const leftEdge = sector.bounds.left;
        const rightEdge = sector.bounds.right;
    
        const bottomEdge = playerGroup.bounds.bottom;
        const topEdge = playerGroup.bounds.bottom - 205;

        // HEIGHT = 205
    
        const topLeft = new paper.Point(leftEdge, topEdge);
    
        const bottomRight = new paper.Point(rightEdge, bottomEdge);
    
        const boundingRectangle = new paper.Path.Rectangle(topLeft, bottomRight)
    
        boundingRectangle.strokeColor = new paper.Color('white');
        boundingRectangle.dashArray = [10, 4];
        boundingRectangle.name = 'bounding';
        boundingRectangle.visible = false;
        boundingRectangle.applyMatrix = false;
    
        //boundingRectangle.rotation = 0;
        //boundingRectangle.rotate(initialGroupData.group.rotation, playerCircle.bounds.center)
    
        masterGroup.addChild(boundingRectangle)
    ...
    return {
        group: noSerialize(playerGroup),
        sector: noSerialize(sector),
        savedPlayerCirclePosition: playerCircle._customPosition,
        savedThroughCirclePosition: throughCircle._customPosition
    }
})

const movePlayers = (extractedData?: Keyframe) => {
    if (!extractedData) {
        return;
    }

    const groupsAndSectors = getGroupsAndSectors().all;

    //console.log({groupsAndSectors})

    for (const groupIndex in extractedData) {
        const keyframedData = extractedData[groupIndex];
        const playerIndex = parseInt(keyframedData.group.name.charAt(keyframedData.group.name.length - 1));

        const innerGroup = groupsAndSectors.find(playerItem => {
            if (!playerItem) return false;
            return playerItem.name === keyframedData.group.name;
        });
        const throughCircle = innerGroup?.children.find(groupChildren => {
            return groupChildren.name === 't-circle';
        }) as CustomPositionItem;
        const playerCircleGroup = innerGroup?.children.find(playerItem => {
            return playerItem.name === `p-circle-group`;
        });
        const playerCircle = playerCircleGroup?.children.find(playerItem => {
            return playerItem.name === `p-circle-${playerIndex}`;
        }) as CustomPositionItem;


        const playerSector = groupsAndSectors.find(playerItem => {
            if (!playerItem) return false;
            return playerItem.name === `p-sector-${playerIndex}`;
        });

        
        if(!innerGroup || !throughCircle || !playerSector || !playerCircleGroup || !playerCircle) {
            console.error('not all player items found');
            console.log({group: innerGroup, throughCircle, playerSector, playerCircleGroup, playerCircle})
            return;
        }
        //console.log({groupData});

        const innerGroupPoint = addDistanceToFieldCenter(new paper.Point(
            keyframedData.group.position.x,
            keyframedData.group.position.y 
        )) 

        const deltaX = innerGroupPoint.x - innerGroup.position.x;
        const deltaY = innerGroupPoint.y - innerGroup.position.y;
        const deltaPoint = new paper.Point(deltaX, deltaY);
        moveHighlight(groupIndex, deltaPoint);

        innerGroup.position.x = innerGroupPoint.x;
        innerGroup.position.y = innerGroupPoint.y;



        innerGroup.rotation = keyframedData.group.rotation;

        const playerNumber = playerCircleGroup.children.find(item => item.name === 'p-number');
        if (playerNumber) {
            playerNumber.rotation = -keyframedData.group.rotation;
        }

       //console.log('ROTATION', keyframedData.group.rotation)

    
        const playerSectorPoint = addDistanceToFieldCenter(new paper.Point(
            keyframedData.sector.position.x,
            keyframedData.sector.position.y 
        )) 

        playerSector.position.x = playerSectorPoint.x;
        playerSector.position.y = playerSectorPoint.y;
        playerSector.rotation = keyframedData.group.rotation;

        checkSectorIntersections(playerSector.id);
    
        //const customPositionPoint = new paper.Point(playerCircle._customPosition.x, playerCircle._customPosition.y)

        playerCircle._customPosition.x = keyframedData.playerCircle.position.x;
        playerCircle._customPosition.y =  keyframedData.playerCircle.position.y;

        //console.log({playerCircle})

        throughCircle._customPosition.x = keyframedData.throughCircle.position.x;
        throughCircle._customPosition.y = keyframedData.throughCircle.position.y; 



        /* const testPoint = customPositionPoint;
        console.log(groupData)
        const test = new paper.Path.Circle(testPoint, 10);
        test.fillColor = new paper.Color('red'); */
    
        //writeVar(`${groupIndex}-circle-x`, playerCircle.x);
        //writeVar(`${groupIndex}-circle-y`, playerCircle.y);
    }
}
...
export const useInitialLoader = routeLoader$(async ({ cookie, redirect, params, pathname }) => {
    ...
    const shardUUID = params.id;
    let shard: ShardRow | null = null;

    if (shardUUID) {
        const { data, error } = await supabase
        .from('shards').select().eq('uuid', shardUUID);
    
        if (error) {
            throw redirect(302, `/shards`);
        }

        const response = data as any as ShardRow[];
        shard = response[0];
    }
...
const getDistanceToFieldCenter = (point: { x: number, y: number}) => {
    const field = findLayer('field');
    if (!field) {
        throw new Error('field not accessible.')
    }
    const centerReferencePoint = field.bounds.center;


    const pointToAdjust = new paper.Point(point.x, point.y);
    const adjusted = pointToAdjust.subtract(centerReferencePoint);
...
const addDistanceToFieldCenter = (point: { x: number, y: number} | paper.Point) => {
    const field = findLayer('field');
    if (!field) {
        throw new Error('field not accessible.')
    }
    ...
    const distanceToAdd = new paper.Point(point.x, point.y);
    const adjusted = centerReferencePoint.add(distanceToAdd);
...
const extractEssentialData = (essentialData: EssentialPlayerData, groupIndicies: string[]) => {
    const extractedData: Keyframe = {};

    for (const groupIndex of groupIndicies) {
        const {
            group: { 
                //@ts-ignore
            ...
            savedPlayerCirclePosition: { x: playerCircleX, y: playerCircleY },
            savedThroughCirclePosition: { x: throughCircleX, y: throughCircleY },
        } = essentialData[groupIndex as keyof EssentialPlayerData];

        console.log(groupRotation)

        const extractedGroup = {
            group: { 
                position: getDistanceToFieldCenter({ x: groupX, y: groupY }), 
                rotation: groupRotation,
                name 
            },
            sector: { 
                position: getDistanceToFieldCenter({ x: sectorX, y: sectorY }) 
            },
            playerCircle: { 
                position: { x: playerCircleX, y: playerCircleY }
            },
            throughCircle: { 
                position: { x: throughCircleX, y: throughCircleY }
            },
        }
        ...
        extractedData[groupIndex] = extractedGroup;
    }

    return extractedData;
}

const saveShardToDatabase = async (shardData: ShardRow) => {

    const { data, error } = await supabase
    .from('shards').update(shardData).eq('uuid', shardData.uuid).select();

    if (error) {
        return null;
    }

    return data as unknown as Partial<ShardRow>[];
} 

type Keyframe = {
    [index: string]: ExtractedPlayerData;
}

type ExtractedPlayerData = { 
    group: {
        /**
         * The distance to the field's center.
         */
        position: {
            x: number;
            y: number;
        ...
        rotation: number;
        /**
         * Identifying dash-separated name.
         */
        name: string
    };
    sector: {
...
type EssentialPlayerData = { 
    [index: number | string]: {
        group: NoSerialize<paper.Group>;
        sector: NoSerialize<paper.Path.Arc>;
...
const interpolateKeyframeDeep = (keyframes: { 
    [timestamp: string]: Keyframe
}, currentTimestamp: string): Keyframe | undefined => {
    const timestampDecimal = parseFloat(currentTimestamp);
    if (keyframes[currentTimestamp]) {
        console.log('is on exact keyframe.')
        return keyframes[currentTimestamp]
    }

    // Get an array of timestamps from the keyframes object
    // These should be integers.
    const keyframedTimestamps = Object.keys(keyframes).map(x => {
        return parseInt(x);
    }).sort((a, b) => a - b);
    console.log({keyframedTimestamps})

    const groupsOnShard = getGroupsAndSectors().groups;

    const finalPosition: any = {};

    const interpolatePosition = (prevPosition: {
        x: number;
        y: number;
    }, nextPosition: {
        x: number;
        ...
        return {
            x: prevPosition.x + (factor * (nextPosition.x - prevPosition.x)),
            y: prevPosition.y + (factor * (nextPosition.y - prevPosition.y)),
        }
    } 
    const interpolateRotation = (prevRotation: number, nextRotation: number, factor: number) => {
        // Calculate the absolute difference between the two angles
        const angleDifference = Math.abs(nextRotation - prevRotation);

        // Check if it's shorter to go in the other direction
        ...
        return prevRotation + (factor * (nextRotation - prevRotation)) % 360;
    };

    for (const group of groupsOnShard) {
        const groupIndex = parseInt(group.name.charAt(group.name.length - 1));

        let prevGroup: ExtractedPlayerData | null = null;
        let nextGroup: ExtractedPlayerData | null = null;

        let prevTimestamp: number | null = null;
        let nextTimestamp: number | null = null;

        for (const keyframedTimestamp of keyframedTimestamps) {
            if (prevGroup && nextGroup) break;

            const frame = keyframes[keyframedTimestamp];
            const group = frame[groupIndex];

            if (!group) continue;

            if (keyframedTimestamp < timestampDecimal) {
                prevGroup = group;
                prevTimestamp = keyframedTimestamp;
            } else if (keyframedTimestamp > timestampDecimal) {
                nextGroup = group;
                nextTimestamp = keyframedTimestamp;
            } else {
                console.error('this case should have been guarded earlier.')
            }
        }

        console.log(prevTimestamp, nextTimestamp, timestampDecimal)

        if (nextGroup && !prevGroup) {
            finalPosition[groupIndex] = nextGroup;
            console.warn('nextGroup is present, prevGroup is not.')
            continue;
        }
        if (!nextGroup && prevGroup) {
            finalPosition[groupIndex] = prevGroup;
            console.warn('nextGroup is not present, prevGroup is.')
            continue;
        }
        ...
        if (prevTimestamp === null || nextTimestamp === null) {
            console.warn('this case should have been guarded earlier.');
            continue;
        }

        for (const item in prevGroup) {
            const itemKey = item as 'group' | 'sector' | 'playerCircle' | 'throughCircle';
            const interpolationFactor = (timestampDecimal - prevTimestamp) / (nextTimestamp - prevTimestamp);
            const interpolated = interpolatePosition(prevGroup[itemKey].position, nextGroup[itemKey].position, interpolationFactor);
            
            const itemPositionalData: {
                position: {
                    x: number, 
                    y: number
                }, 
            ...
            if (itemKey === 'group') {
                itemPositionalData.name = prevGroup[itemKey].name
                itemPositionalData.rotation = interpolateRotation(prevGroup[itemKey].rotation, nextGroup[itemKey].rotation, interpolationFactor);
            //console.log({beforeInterpolation: prevGroup[itemKey].position, interpolated});
            }

            if (!finalPosition[groupIndex]) {
                finalPosition[groupIndex] = {
                    [itemKey]: itemPositionalData,
                }
            } else {
                finalPosition[groupIndex][itemKey] = {
                    position: interpolated
                }
            }
    ...
    return finalPosition as Keyframe;
}


/**
 * 
 * @param keyframes pass a reference to the `animationStore.keyframes` proxy.
 * @param currentTimestamp the floating-point string to use as a timestamp.
 * @returns the keyframe to move to.
 */
const interpolateKeyframe = (keyframes: { 
    [timestamp: string]: Keyframe
}, currentTimestamp: string): Keyframe | undefined => {
    // Convert currentTimestamp to a number
    const timestamp = parseFloat(currentTimestamp);

    // Get an array of timestamps from the keyframes object
    const timestamps = Object.keys(keyframes).map(parseFloat).sort((a, b) => a - b);
    ...
    // Check if we found two valid keyframes
    if (prevTimestamp === undefined || nextTimestamp === undefined) {
        return keyframes[currentTimestamp];
    }

    // Calculate the interpolation factor
    const interpolationFactor = (timestamp - prevTimestamp) / (nextTimestamp - prevTimestamp);
    ...
    const interpolatePosition = (prevPosition: {
        ...
        return {
            x: prevPosition.x + (interpolationFactor * (nextPosition.x - prevPosition.x)),
            y: prevPosition.y + (interpolationFactor * (nextPosition.y - prevPosition.y)),
        }
    } 
    const interpolateRotation = (prevRotation: number, nextRotation: number) => {
        // Calculate the absolute difference between the two angles
        const angleDifference = Math.abs(nextRotation - prevRotation);
        ...
        // Interpolate the angles
        return prevRotation + (interpolationFactor * (nextRotation - prevRotation)) % 360;
    };

    // Interpolate the position
    const prevData = keyframes[prevTimestamp.toString()];
    const currData = keyframes[currentTimestamp];
    const nextData = keyframes[nextTimestamp.toString()];
    ...
    for (const groupIndex in prevData) {
        const prevGroup = prevData[groupIndex];
        const nextGroup = nextData[groupIndex]; 
        ...
        for (const item in prevGroup) {
            const itemKey = item as 'group' | 'sector' | 'playerCircle' | 'throughCircle';
            const interpolated = interpolatePosition(prevGroup[itemKey].position, nextGroup[itemKey].position);
            ...
            if (itemKey === 'group') {
                itemPositionalData.name = prevGroup[itemKey].name
                itemPositionalData.rotation = interpolateRotation(prevGroup[itemKey].rotation, nextGroup[itemKey].rotation);
            //console.log({beforeInterpolation: prevGroup[itemKey].position, interpolated});
            }
...
const roundToMultipleFloor = (num: number, multiple: number) => {
...
const roundToMultipleCeiling = (num: number, multiple: number) => {
    return Math.ceil(num / multiple) * multiple;
}  

const getKeyframeMeasures = () => {
    const boxWidth = 60;
    const boxMargin = 4;

    const distanceBetweenKeyframes = boxWidth + boxMargin;

    const stepsBetweenKeyframes = 4;

    const stepWidth = distanceBetweenKeyframes / stepsBetweenKeyframes;
...
const highlightPlayer = (groupIndex: number | string) => {
    const layer = findLayer('player-layer');

    const parsedGroupIndex = typeof groupIndex === 'string' ? groupIndex : `${groupIndex}`;
    const masterGroup = layer?.children.find(item => {
        return item.name?.includes(parsedGroupIndex)
    });

    if (masterGroup && masterGroup.children) {
        const bounding = masterGroup.children.find(item => item.name === 'bounding');
        if (bounding) {

            bounding.visible = true;
            console.log('setting visibnle')
        }
    } else {
        console.log(masterGroup)
    }
}
const moveHighlight = (groupIndex: number | string, delta: paper.Point, visible: boolean = false) => {
    const layer = findLayer('player-layer');
    ...
    if (masterGroup && masterGroup.children) {
        const bounding = masterGroup.children.find(item => item.name === 'bounding');
        if (bounding) {
            //console.log(bounding.position)
            bounding.visible = visible;
            bounding.position = bounding.position.add(delta);
        }
    }    
}
const unHighlightPlayer = (groupIndex: number | string) => {
    const layer = findLayer('player-layer');
...
/**
 * 
 * @param store pass a reference to the `animationStore` proxy.
 * @returns the keyframe to move to.
*/
const checkKeyframeControlBoxes = (store: AnimationJson) => {
    const {
        distanceBetweenKeyframes, 
        stepWidth
    } = getKeyframeMeasures();

    const needle = document.querySelector('.needle-move') as HTMLElement;
    const transformRegex = /translateX\((\d+(?:\.\d+)?)px\)/;
    const matches = needle.style.transform.match(transformRegex);
    let transformValue = 0;
    if (matches) {
        transformValue = parseFloat(matches[1]);
    }
    

    const rounded = roundToMultiple(transformValue, stepWidth);
    const isOnKeyframe = rounded % distanceBetweenKeyframes === 0;

    const leftControlArrows = document.querySelectorAll('.move-left.keyframe-control');
    const rightControlArrows = document.querySelectorAll('.move-right.keyframe-control');
    
    if (rounded <= 0) {
        leftControlArrows.forEach(arrow => arrow.classList.add('disable'))
    } else {
        leftControlArrows.forEach(arrow => arrow.classList.remove('disable'))
    }

    console.log(getKeyframeMeasures())

    if (rounded >= distanceBetweenKeyframes * (store.cellCount - 1)) {
        rightControlArrows.forEach(arrow => arrow.classList.add('disable'))
    } else {
        rightControlArrows.forEach(arrow => arrow.classList.remove('disable'))
    }

    const timestamp = (rounded / distanceBetweenKeyframes)

    const keyframeControlBoxes = document.querySelectorAll('.place.keyframe-control > .animation-grid-box.control')
    keyframeControlBoxes.forEach(controlBox => {
        const box = controlBox as HTMLElement;
        if (!box.dataset.groupIndex) return;

        if (isOnKeyframe) {
            const isKeyframed = store.keyframes[timestamp]?.[box.dataset.groupIndex];
            console.log({isKeyframed})
            if (isKeyframed) {
                box.classList.add('fill')
            } else {
                box.classList.remove('fill')
            }
            box.dataset.time = `${timestamp}`;
        } else {
            box.classList.remove('fill')
        }
...
const animationSessionStore = {
    timelinesEditing: [] as string[],
    currentTimestamp: 0
}
...
export default component$(() => {
    ...
    const currentPlayerStore = useStore({
        essentialData: {} as EssentialPlayerData
    })
    ...
    const initialLoader = useInitialLoader();
    const shard = initialLoader.value.shard;
    if (!shard) {
        throw new Error('shard undefined - should be defined.')
    }
    const shardPath = initialLoader.value.pathname;

    const shardStore = useStore({
        shard
    })

    const saveShardHandler = $((shardData: ShardRow) => {
        const updateTimestamp = dayjs().format();
        shardStore.shard.updated_at = updateTimestamp;

        shardData.updated_at = updateTimestamp;
        return saveShardToDatabase(shardData)
    })
    ...
    const addOffensePlayer = $(async (initialGroupData?: ExtractedPlayerData) => {

        const newPlayerData: DrawPlayerProps = {
            point: paper.view.center.add(new paper.Point(0, 50)),
            radius: 30,
            sectorAngle: offenseSectorAngle.value,
            offense: true,
            index: playerCount.value,
            theme: theme.value,
            initialGroupData: cloneProxy(initialGroupData)
        }

        const pData = await drawPlayer(newPlayerData)
        currentPlayerStore.essentialData[playerCount.value] = pData;

        playerCount.value++;

        return pData;
    })
    const addDefensePlayer = $(async (initialGroupData?: ExtractedPlayerData) => {
        ...
        console.log('adding defense', {initialGroupData, newPlayerData})
    ...
    const initialFieldOrientation = shard.field_orientation as 'horizontal' | 'vertical';
    const fieldOrientation = useSignal(initialFieldOrientation);
    ...
    //const animation = shard.animation as AnimationJson;

    const animationStore = useStore(() => {

        const clone = structuredClone(shard.animation as AnimationJson);

        if (!clone.playerCount) {
            clone.playerCount = {
                offense: [],
                defense: []
            };
        }
        if (!clone.playerCount.offense) {
            clone.playerCount.offense = [];
        }
        if (!clone.playerCount.defense) {
            clone.playerCount.defense = [];
        }
    ...
    useVisibleTask$(async () => {
        ...
        //@ts-ignore
        window.createTestPoint = (x: any, y: any) => {
            const testPoint2 = new paper.Point(x, y);
            const test2 = new paper.Path.Circle(testPoint2, 10);
            test2.fillColor = new paper.Color('red');
        }
        ...
        try {
            ...
            if (canvas) {
                ...
                paper.view.onResize = (e: any) => {
                    /* const newWidth = e.size.width;
                    const oldWidth = (e.size.width - e.delta.width);
                    const factor = (newWidth - oldWidth) / 2; */
                    
                    //console.log({factor, delta: e.delta.width})
                    const something = e.delta.width * zoom.value;
                    let pad = 0.4;
                    if (e.delta.width < 0) pad = -pad;

                    paper.view.translate(new paper.Point(something + pad, 0))
                }
                ...
                everythingElseLayer.name = 'player-layer';
                everythingElseLayer.activate();

                const firstKeyframeProxy = animationStore.keyframes[Object.keys(animationStore.keyframes)[0]] as Keyframe;
                console.log({firstKeyframeProxy})

                //const playerAddCalls = [];

                const groupNames = [
                    ...animationStore.playerCount.offense, 
                    ...animationStore.playerCount.defense
                ].sort((a, b) => {
                    const previousIndex = parseInt(a.charAt(a.length - 1))
                    const nextIndex = parseInt(b.charAt(b.length - 1))
                ...
                for(const groupName of groupNames) {
                    const groupIndex = parseInt(groupName.charAt(groupName.length - 1))
                    const keyframedData: ExtractedPlayerData | undefined = firstKeyframeProxy?.[groupIndex];
                    console.log({groupIndex, keyframedData})
                    if (groupName.includes('offense')) {
                        await addOffensePlayer(keyframedData)
                    } else {
                        await addDefensePlayer(keyframedData)
                    }
                }

                checkKeyframeControlBoxes(animationStore)
            }
        } catch (error) {
            console.error(error)
        } 
        ...
        const plainWheelHandler = async (e: WheelEvent) => {
            const isOnCanvas = (e.target as HTMLElement).id === 'canvas';
        ...
        const globalKeyDownHandler = (e: KeyboardEvent) => {
            if (document.body.classList.contains('disable-handlers')) return;
    ...
    const addKeyframeHandler = $(async (e: any) => {
        console.log(e)
        animationStore.cellCount += 1;

        shard.animation = animationStore;
        saveShardHandler(shard);
    })

    const placeKeyframeHandler = $(async (e: any, essentialData: EssentialPlayerData) => {
        const target = e.target as HTMLElement;
        const box = target.closest('.animation-grid-box') as HTMLElement;
        if (!box) return console.error('no box');

        const timestamp: string = box.dataset.time!;
        const groupIndex: string = box.dataset.groupIndex!;
        //const animationTimeline = document.querySelector('.animation-timeline-keyframes') as HTMLElement;

        if (box.classList.contains('fill')) {
            console.log('deleting:', animationStore.keyframes[timestamp][groupIndex])
            delete animationStore.keyframes[timestamp][groupIndex];

            if (Object.keys(animationStore.keyframes[timestamp]).length < 1) {
                delete animationStore.keyframes[timestamp];
            }
            ...
            shard.animation = animationStore;
            checkKeyframeControlBoxes(animationStore);
            await saveShardHandler(shard); 
            
            return;
        } 
    
        const activeIndicies = [groupIndex];
        const extracted = extractEssentialData(essentialData, activeIndicies)

        for(const index of activeIndicies) {
            if (animationStore.keyframes[timestamp]) {
                console.log('adding to existing timestamp', timestamp)
                animationStore.keyframes[timestamp][index] = extracted[index];
            } else {
                console.log('creating timestamp for keyframe', timestamp)
                animationStore.keyframes[timestamp] = {
                    [index]: extracted[index]
                };
            }

            const group = findItemByName(extracted[index].group.name)
            if (group) {
                group.strokeColor = new paper.Color('red');
                group.strokeWidth = 2;
            }

        }
        //animation.cellCount = animationTimeline.querySelectorAll('.animation-grid-box[index]').length;
    ...
    const expandKeyframeControls = $((e: any) => {
        const target = e.target as HTMLElement;
        const timeline = target.closest('.animation-timeline') as HTMLElement;
        if (!timeline || !timeline.dataset.groupIndex) return;
        const groupIndex = timeline.dataset.groupIndex;

        const editingArray = animationSessionStore.timelinesEditing;

        if (timeline.classList.contains('edit')) {
            const indexOfTimeline = editingArray.indexOf(groupIndex)
            editingArray.splice(indexOfTimeline, 1)
            timeline.classList.remove('edit');
        } else {
            editingArray.push(groupIndex);
            timeline.classList.add('edit');
        }

        console.log({animationSessionStore})
    })

    const moveKeyframe = $((event: QwikMouseEvent<HTMLDivElement, MouseEvent>, element: HTMLDivElement) => {
        const target = element as HTMLElement;
        const direction = target.classList.contains('move-left') ? 'left' : 'right';
        const needle = document.querySelector('.needle-move') as HTMLElement;

        if (target.classList.contains('disable')) return;

        const { distanceBetweenKeyframes } = getKeyframeMeasures();

        const previousTransformValue = needle.dataset.position ? parseFloat(needle.dataset.position) : 0;
        let nearestKeyframeTransformValue;
        let delta = distanceBetweenKeyframes;

        if (previousTransformValue !== 0) {
            nearestKeyframeTransformValue = roundToMultipleCeiling(previousTransformValue, distanceBetweenKeyframes)

            if (direction === 'left') {
                nearestKeyframeTransformValue = roundToMultipleFloor(previousTransformValue, distanceBetweenKeyframes)
                delta = -delta;
            }
        } else {
            nearestKeyframeTransformValue = 64;
        }

        console.log({nearestKeyframeTransformValue})

        let transformValue = nearestKeyframeTransformValue;

        if (previousTransformValue === nearestKeyframeTransformValue) {
            transformValue += delta;
        }
        
        needle.style.transform = `translateX(${transformValue}px)`
        needle.dataset.position = `${transformValue}`

        const timestamp = (transformValue / distanceBetweenKeyframes)

        checkKeyframeControlBoxes(animationStore)
        const intPos = interpolateKeyframe(animationStore.keyframes, `${timestamp}`)
        if (intPos) {
            console.log('INTPOS FOUND')
            movePlayers(intPos)
        }
    })
    ...
    const AnimationTimelines = ((cellCount: number | undefined) => {
        console.log('RENDER')
        console.log(`Shard:`, { shard })

        const TimelinesArray: JSX.Element[] = [];
        ...
        console.log(`Generating ${cellCount} cells.`)

        const createPlayerTimeline = (groupName: string) => {
            const groupIndex = parseInt(groupName.charAt(groupName.length - 1))

            const TimelineChildren = [];

            const offense = groupName.includes('offense');

            const labelBox = (
                <div key={'animation-group-label'} class="animation-label-box add">
                    <div class={`label-wrap ${offense ? 'offense' : 'defense'}`}>
                        <div class="label-condensed-wrap">
                            <div class="label-expand-arrow" onClick$={expandKeyframeControls}>
                            ...
                            <div class="label-value">Player {groupIndex + 1}</div>
                        </div>

                        <div class="label-expanded-wrap">
                            <div class="keyframing-wrap">
                                <div class="move-left keyframe-control disable" onClick$={moveKeyframe}>
                                    <svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="css-i6dzq1"><polyline points="6 9 12 15 18 9"></polyline></svg>
                                </div>
                                <div class="place keyframe-control">
                                    <div class={`animation-grid-box control `} 
                                        key={'animation-keyframe-control'} 
                                        
                                        data-time={0}
                    
                                        data-group-index={groupIndex}
                                    >
                                        <div class="keyframe-slot" onClick$={(e) => {
                                            placeKeyframeHandler(e, currentPlayerStore.essentialData)
                                        }}>
                                ...
                                <div class="move-right keyframe-control" onClick$={moveKeyframe}>
                                    <svg viewBox="0 0 24 24" width="24" height="24" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="css-i6dzq1"><polyline points="6 9 12 15 18 9"></polyline></svg>
                                </div>
                            </div>
                        </div>
            ...
            TimelineChildren.push(labelBox);

            for (let i = 0; i < cellCount!; i++) {
                let identifier = '';
                if (i === 0) identifier = 'start';
                if (i === cellCount) identifier = 'end';
    
                const isKeyframed = animationStore.keyframes[i]?.[groupIndex];
    
                const animationBox = (
                    <div 
                        key={i} 
                        class={`animation-grid-box ${identifier} ${isKeyframed ? 'fill' : ''}`} 
                        data-index={i} 
                        data-time={i}
                ...
                TimelineChildren.push(animationBox)
            }
    
            const adderBox = (
                <div onClick$={(e) => {
                    addKeyframeHandler(e)
                }} key={'adder'} class="animation-grid-box add">
                    <div class="add-text">
                        Add
                    </div>
                </div>
            )
            TimelineChildren.push(adderBox);

            const indexOfTimeline = animationSessionStore.timelinesEditing.indexOf(`${groupIndex}`)
            console.log({indexOfTimeline})

            const Timeline = (
                <div 
                    class={`animation-timeline timeline-${groupIndex} ${indexOfTimeline >= 0 ? 'edit' : ''}`}
                    data-group-index={groupIndex}
                >
                    {TimelineChildren}
                </div>
        ...
        for(const groupName of animationStore.playerCount.offense) {
            createPlayerTimeline(groupName)
        }
        for(const groupName of animationStore.playerCount.defense) {
            createPlayerTimeline(groupName)
        }

        console.log(TimelinesArray.length)
    ...
    const isAnimationPlaying = useSignal(false)

    const startScrubNeedleHandler = $(() => {
        console.log('adding handler')
        const needle = document.querySelector('.needle-move') as HTMLElement;
        isAnimationPlaying.value = false;

        const timeline = document.querySelector('.animation-timeline-inner') as HTMLElement;

        const endScrubNeedleHandler = () => {
            //console.log(e)
            document.removeEventListener('mousemove', scrubMoveNeedleHandler)
            document.removeEventListener('mouseup', endScrubNeedleHandler);
        }

        let previousRoundedNeedleTransform = needle.dataset.position ? parseFloat(needle.dataset.position) : 0;
    
        const scrubMoveNeedleHandler = (e: MouseEvent) => {
            timeline.querySelector(`[data-index].active`)?.classList.remove('active');

            const initialLeftOffset = 128; // Same as left: ...px in CSS
            const needleHeadWidth = 20;
            let transformValue = (
                e.clientX - initialLeftOffset 
                - (needleHeadWidth / 2)
            );
            if (transformValue < 0) transformValue = 0;       
            const maxPixelPosition = ((animationStore.cellCount - 1) * 64);
            if (transformValue >= maxPixelPosition) {
                console.log('max.')
                return;
            }

            const { stepWidth } = getKeyframeMeasures();
            const rounded = roundToMultiple(transformValue, stepWidth);

            checkKeyframeControlBoxes(animationStore)

            if (rounded === previousRoundedNeedleTransform) {
                return;
            }
            previousRoundedNeedleTransform = rounded;

            needle.style.transform = `translateX(${rounded}px)`
            needle.dataset.position = `${rounded}`

            const needlePosition = parseFloat(needle.dataset.position);

            const cellNumber = (
                (needlePosition / (animationStore.cellCount * 64)) 
                * animationStore.cellCount
            );

            const closestCell = Math.round(cellNumber);

            const intPos = interpolateKeyframe(animationStore.keyframes, `${cellNumber}`);
            ...
            timeline.querySelector(`[data-index="${closestCell}"]`)?.classList.add('active');
        }

        //console.log(e)
        document.addEventListener('mouseup', endScrubNeedleHandler);
        document.addEventListener('mousemove', scrubMoveNeedleHandler)
    })

    const playAnimationHandler = $(async () => {
        if (isAnimationPlaying.value === true) {
            isAnimationPlaying.value = false;
            return
        }
        ...
        const timeline = document.querySelector('.animation-timeline-inner') as HTMLElement;
        const needle = document.querySelector('.needle-move') as HTMLElement;
        const initialNeedlePosition = needle.dataset.position;

        let pixelPosition = ((animationStore.cellCount - 1) * 64);
        console.log({initialNeedlePosition, pixelPosition})

        let initialTimestamp = 0;
        let currentTimestamp = 0; 

        if (initialNeedlePosition) {
            if (parseFloat(initialNeedlePosition) >= pixelPosition) {
                needle.style.removeProperty('transform');
            } else {
                const currentCell = ((parseFloat(initialNeedlePosition) / pixelPosition) * animationStore.cellCount)
                currentTimestamp = currentCell;
                initialTimestamp = currentCell;

                pixelPosition += currentCell * 64;
            }
        }

        const initialIntPos = interpolateKeyframeDeep(animationStore.keyframes, currentTimestamp.toString())
        ...
        /* const resetNeedle = () => {
            anime({
                targets: needle,
                translateX: 0,
                duration: 1000 * animationStore.cellCount / 10,
                easing: 'easeInOutCubic',
            })

            movePlayers(animationStore.keyframes[Object.keys(animationStore.keyframes)[0]] as Keyframe, 1000)
        } */

        const animationDuration = (animationStore.cellCount - 1 - initialTimestamp)*1000;
        console.log({animationDuration, pos: pixelPosition / 64});

        anime({
            targets: needle,
            translateX: pixelPosition,
            duration: animationDuration,
            easing: 'linear',

            update: function(anim) {
                if (isAnimationPlaying.value === false) {
                    console.groupCollapsed(`ENDING: animation stopped.`)
                    console.log({})
                    console.groupEnd();
                    anim.pause();
                ...
                if (matches) {
                    const extractedNumber = parseFloat(matches[1]);
                    needle.dataset.position = `${extractedNumber}`;
                }



                // Floating number e.g. 5.73, 2.24
            
                const cellNumber = (
                    (anim.progress / 100 * (animationStore.cellCount - 1)) 
                    + initialTimestamp
                );

                console.log(cellNumber);

                const currentCell = Math.floor(cellNumber);
                ...
                const intPos = interpolateKeyframeDeep(animationStore.keyframes, `${cellNumber}`);
                if (intPos) {
                    console.log('INTPOS', {intPos})
                    movePlayers(intPos)
                } else {
                    console.groupCollapsed(`ENDING: intPos does not exist.`)
                ...
                if (currentTimestamp !== currentCell) {
                    currentTimestamp = currentCell;
                }
            },
            complete: () => {
    ...
    const skipBackAnimationHandler = $(() => {
        const needle = document.querySelector('.needle-move') as HTMLElement;
        isAnimationPlaying.value = false;

        needle.style.removeProperty('transform');
        needle.dataset.position = `0`;

        setTimeout(() => {
            needle.style.removeProperty('transform');
            needle.dataset.position = `0`;

            const intPos = interpolateKeyframeDeep(animationStore.keyframes, `${0}`);
            if (intPos) {
                console.log('INTPOS', {intPos})
                movePlayers(intPos)
            }
    ...
    const skipForwardAnimationHandler = $(() => {
        const needle = document.querySelector('.needle-move') as HTMLElement;
        isAnimationPlaying.value = false;
        
        setTimeout(() => {
            const pixelPosition = ((animationStore.cellCount - 1) * 64);
            needle.style.transform = `translateX(${pixelPosition}px)`;
            needle.dataset.position = `${pixelPosition}`;

            const intPos = interpolateKeyframeDeep(animationStore.keyframes, `${(animationStore.cellCount - 1)}`);
            if (intPos) {
                console.log('INTPOS', {intPos})
                movePlayers(intPos)
            }
    ...
    const needleRef = useSignal<HTMLElement>();

    useVisibleTask$(({ cleanup }) => {
        if (needleRef.value) {
    
            needleRef.value.addEventListener('mousedown', startScrubNeedleHandler);
            cleanup(() => {
                needleRef.value!.removeEventListener('mousedown', startScrubNeedleHandler);
            });
        }
    })

    const generateTimestamps = (cellCount: number) => {
        const timestamps = [];
        ...
        for(let i = 0; i < cellCount; i++) {
            const timestampBox = (
                <div 
                    ...
                    data-time={i}
                >
                    <div class="timestamp">{i}s</div>
                </div>
            )

            timestamps.push(timestampBox)                     
        }

        return timestamps
    }

    const toggleTimeline = $((event: any, element: any) => {
        const target = element as HTMLElement;
        const toolbar = target.closest('.timeline-toolbar-wrapper');
        toolbar?.classList.toggle('open');
    })

    /*OVERLAY DOM HANDLERS*/
    const globalPrefersReducedMotion = useSignal(false);
    
    const showOverlayHandler = $((actionString: 'Edit' | 'Create' | 'Copy') => {
        disableHandlers();
    ...
    const editShardHandler = $(() => {
        showOverlayHandler('Edit');
    })

    const deleteShardHandler = $(() => {
        console.warn('no support for that yet.')
    })
    ...
    return (
        ...
        {shardPath ? <Navbar path={shardPath} customLastCrumb={shard.title || 'Untitled'} /> : <></>}
        ...
        <div class={`sandbox-wrap ${currentMouseTool.value}`}>
            ...
            <div class="sandbox-toolbar-outer">
                <div class="sandbox-toolbar-inner">
                    ...
                    <div class="content-control-wrap button-group">
                        <div class="control-button-wrap standard-sandbox-button" onClick$={toggleButtonDrop} >
                            ...
                            <div class="add-player-drop button-drop">
                                <button class="standard-sandbox-button" data-offense="true" onClick$={async () => {
                                    const playerData = await addOffensePlayer()
                                    if (!playerData?.group?.name) return console.error('no group name');
                                    animationStore.playerCount.offense.push(playerData.group.name);

                                    shard.animation = animationStore;
                                    saveShardHandler(shard);
                                }}>Offense</button>
                                <button class="standard-sandbox-button" onClick$={async () => {
                                    const playerData = await addDefensePlayer()
                                    if (!playerData?.group?.name) return console.error('no group name');
                                    animationStore.playerCount.defense.push(playerData.group.name);
                            ...
                            <div class="add-player-drop button-drop">
                                ...
                                <button class="field-orientation standard-sandbox-button" onClick$={() => {
                                    if (fieldOrientation.value === 'horizontal') {
                                        ...
                                        shard.field_orientation = 'vertical';
                                        saveShardHandler(shard);
                                    } else {
                                        fieldOrientation.value = 'horizontal';
                                        shard.field_orientation = 'horizontal';
                                        saveShardHandler(shard);
                                    }
                                }} >Rotate Field</button>
                            </div>
                        ...
                        <div class="control-button-wrap standard-sandbox-button" onClick$={toggleButtonDrop} >
                            ...
                            <div class="add-player-drop button-drop">
                                ...
                                {/* <button class="save-shard standard-sandbox-button" onClick$={exportCanvasImage} >Save</button> */}
                                <button class="get-positionals-button standard-sandbox-button">Get positionals</button>
                            </div>
                        </div>

                        {/* <div class="control-button-wrap standard-sandbox-button change-theme" onClick$={toggleButtonDrop} >
                    ...
                    <div class="shard-meta button-group">
                        <div class="meta-button-wrap standard-sandbox-button" onClick$={toggleButtonDrop} >
                            <button class="edit-shard button">Edit Shard</button>
                            ...
                            <div class="add-player-drop button-drop">
                                <button class="standard-sandbox-button" data-offense="true" onClick$={editShardHandler}>Edit Details</button>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="shard-meta-outer">
                    <div class="shard-meta-inner">
                        <div class="shard-last-saved-text">
                            {shardStore.shard.updated_at ? 
                            <>
                                <span>Auto-saved {dayjs(shardStore.shard.updated_at).fromNow()}</span>
                                <span> | </span>
                                <span> {dayjs(shardStore.shard.updated_at).format('M.D.YYYY h:mm A')} </span>
                            </>
                            : <></>
                            }
                        </div>
            ...
            <div class="sandbox-animation-outer">
                <div class="sandbox-animation-inner">

                    <div class="animation-timeline-outer">

                        <div class="timeline-toolbar-wrapper">
                            <div class="animation-timeline-toggle animation-toolbar-toggle">
                                <svg onClick$={toggleTimeline} viewBox="0 0 24 24" width="20" height="20" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="css-i6dzq1"><polyline points="6 9 12 15 18 9"></polyline></svg>
                            </div>
                            <div class="animation-timeline-label animation-toolbar-label">ANIMATION</div>

                            <div class="playback-controls-outer">
                                <div class="playback-controls-inner">

                                    <div class="playback-control-button" onClick$={skipBackAnimationHandler}>
                                        <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="css-i6dzq1"><polygon points="19 20 9 12 19 4 19 20"></polygon><line x1="5" y1="19" x2="5" y2="5"></line></svg>
                                    </div>
                                    <div class={`playback-control-button play-animation-button ${isAnimationPlaying.value ? 'pause' : 'play'}`} onClick$={playAnimationHandler}>
                                        {isAnimationPlaying.value ? 'Pause' : 'Play'}

                                        {isAnimationPlaying.value ? 
                                        <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="css-i6dzq1"><rect x="6" y="4" width="4" height="16"></rect><rect x="14" y="4" width="4" height="16"></rect></svg>
                                        :
                                        <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="css-i6dzq1"><polygon points="5 3 19 12 5 21 5 3"></polygon></svg> 
                                        }
                                    </div>
                                    <div class="playback-control-button" onClick$={skipForwardAnimationHandler}>
                                        <svg viewBox="0 0 24 24" width="16" height="16" stroke="currentColor" stroke-width="2" fill="none" stroke-linecap="round" stroke-linejoin="round" class="css-i6dzq1"><polygon points="5 4 15 12 5 20 5 4"></polygon><line x1="19" y1="5" x2="19" y2="19"></line></svg>
                                    </div>
                        ...
                        <div class="animation-timeline-inner custom-scrollbar">
                            <div class="animation-timeline-meta">
                                <div class="current-timestamp-display">
                                    {animationSessionStore.currentTimestamp}
                                </div>
                                <div class="timestamps">
                                    {generateTimestamps(animationStore.cellCount)}
                                </div>
                            </div>
                            <div class="animation-timeline-keyframes">
                                {AnimationTimelines(animationStore.cellCount)}
                            </div>

                            <div class="needle-move animation-needle-inner">
                                <div class="needle-head" ref={needleRef}></div>
            ...
            <div class="drill-edit-overlay-inner">
                <div class="drill-edit-meta-form">
                    <div class="top-actions-meta-form">
                        ...
                        <button class={`creation-delete-button show`} onClick$={deleteShardHandler}>Delete Shard</button>
                    </div>
                    <div class="drill-edit-meta-form-data">
                        <div class="drill-edit-title">
                            {`Edit "${shard.title}"`}
                        </div>
                        <div class="drill-edit-description">Edit this Shard</div>
                        
                        <div class="title-label">Title</div>
                        <input name="drill-title-input" class="drill-title-input" type="text" value={shard.title || ''}
                        onInput$={(e) => {
                            const titleString = (e.target as HTMLInputElement).value;
                            shard.title = titleString;
                        }}
                        />
                        ...
                        class="drill-description-input"  value={shard.description || ''} />

                        <button class="creation-save-button" onClick$={async () => {
                            await saveShardHandler(shard)
                            hideOverlayHandler();
                        }}>Save Shard</button>
                    </div>
...
export const head: DocumentHead = ({resolveValue}) => {
    ...
    return {
        title: `${loader?.shard?.title || 'Untitled'} | Front Cone`,
        meta: [
            {
                name: 'description',
                content: `${loader?.shard?.description || 'No description'}`,
            },
        ],
    };
}